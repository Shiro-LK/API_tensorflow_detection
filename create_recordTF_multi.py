...
def get_data(input_path, path='', channels=4):
    ...
    with open(input_path,'r') as f:
        ...
        for line in f:
            ...
            basename = filename[0][:filename[0].rfind('_')+1]
...
def create_one_tf_example(data, class_map, format_img = 'png', path='', channels=4):
    '''
...
def __main__(filename = 'raccoon_dataset.txt', output_label = 'raccoon_label.pbtxt', output_train = 'train.record', output_test = 'test.record', num_channels=4, path=''):
    '''
...
##### MAIN ######
parser = OptionParser()
parser.add_option('-f', "--filename", dest="filename", help='dataset filename', default='jarvis_dataset.txt')
parser.add_option("--train_output", dest="train_output", help="name of the train record file", default='trainJarvis.record')
parser.add_option("--test_output", dest="test_output", help="name of the test record file", default='testJarvis.record')
parser.add_option("--label_output", dest="label_output", help="name of the label pbtxt file", default='labelJarvis.pbtxt')
...
path = options.path
__main__(filename=filename, output_train = output_train, output_test=output_test, output_label=output_label, num_channels=num_channels, path=path)
